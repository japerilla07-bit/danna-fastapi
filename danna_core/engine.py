...
def _bet_advice_from_analysis(analysis: dict, params: dict, coherence: dict) -> dict:
    ...
    for key in ["docenas", "columnas", "color", "paridad", "rango", "max_conf"]:
        ...
        _grp_p1 = None  # VALIDACION: prob del grupo individual mas fuerte (solo doc/col)
        _grp_p2 = None  # VALIDACION: prob del segundo grupo
            ...
            if key in ("docenas", "columnas"):
                ...
                if isinstance(top2, list) and len(top2) >= 2:
                    try:
                        g1, p1 = top2[0][0], float(top2[0][1])
                        g2, p2 = top2[1][0], float(top2[1][1])
                        _grp_p1 = float(p1)   # guardar antes de que se pierdan
                        _grp_p2 = float(p2)
                        pick = f"{g1} / {g2}"
                        p = float(p1 + p2)
                        base = 24.0 / 37.0
        ...
        advice[key] = {
            ...
            # VALIDACION (aditivo): prob de cada grupo individual para
            # docenas/columnas. p (arriba) es la SUMA de ambos; estos son
            # los grupos por separado (rango real). None para otras categorias.
            # No afecta ninguna decision — solo se exponen para medir.
            "p1": _grp_p1,
            "p2": _grp_p2,
        }
